...
<?php
	...
	echo '<p>' . metadata($collection, array('Dublin Core', 'Description')) . '</p>';
        ...
        for ($i=0; $i < $num_of_collection_items; $i++) {
            echo '<li>';
            echo '<a href="'.$collection_item_list[$i]['link'].'" rel="tooltip" title="'.$collection_item_list[$i]['name'].'">'.$collection_item_list[$i]['thumb'].'<br/><div style="text-align: center;">' .$collection_item_list[$i]['name']. '</div></a>';
            echo '</li>';
        }
    ...
    for ($k=1; $k <= $num_of_collections; $k++) {
        ...
        echo "infiniteLoop: false, " . PHP_EOL;
        echo " }); " . PHP_EOL;
    }
